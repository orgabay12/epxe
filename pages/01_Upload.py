...
tab_web, tab_manual, tab_excel, tab_image = st.tabs(["🌐 Web Extract", "📝 Manual Entry", "📄 AI Excel Processing", "🤖 AI Image Extraction"])
            ...
            try:
                ...
                for mode, chunk in app.stream(inputs, stream_mode=["custom", "values"]):
                    if mode == "custom":
                        ...
                        if message:
                            ...
                            log_placeholder = log_placeholder.empty()
                ...
                for tx in categorized_transactions:
                    ...
                    if not db.transaction_exists_by_identifier(merchant, tx.amount, tx.date):
                        ...
                        logs.insert(0, f"✅ Added: {merchant} - ₪{tx.amount}")
                        added_count += 1
                    else:
                        logs.insert(0, f"⏭️ Skipped duplicate: {merchant} - ₪{tx.amount}")
                        skipped_count += 1
                    log_placeholder.code("\n".join(logs))
        ...
        try:
            ...
            if st.button("Process with AI"):
                ...
                with st.status("📄 Processing Excel file...", expanded=True) as status:
                    try:
                        logs.insert(0, f"📄 Reading Excel file... Found {len(df)} rows")
                        
                        # Convert entire DataFrame to text for the AI agent
                        file_content = df.to_string(index=False)
                        logs.insert(0, "🔄 Converting file content for AI analysis...")
                        ...
                        if not categorized_transactions:
                            logs.insert(0, "⚠️ No transactions extracted")
                            ...
                            logs.insert(0, f"🔄 Processing and saving {len(categorized_transactions)} transaction(s)...")
                            ...
                            for i, transaction in enumerate(categorized_transactions):
                                try:
                                    ...
                                    if not db.transaction_exists_by_identifier(merchant, float(amount), date):
                                        ...
                                        logs.insert(0, f"✅ Added: {merchant} - ₪{amount} ({category})")
                                        added_count += 1
                                    else:
                                        logs.insert(0, f"⏭️ Skipped duplicate: {merchant} - ₪{amount}")
                                        skipped_count += 1
                                    ...
                                    errors.append(error_msg)
                                    logs.insert(0, f"❌ {error_msg}")
                                    skipped_count += 1
                            
                            logs.insert(0, f"🎉 Processing complete! Added {added_count}, skipped {skipped_count}")
                            status.update(label=f"✅ Complete! Added {added_count}, skipped {skipped_count}", state="complete", expanded=True)
                            log_placeholder.code("\n".join(logs))
...
with tab_image:
    ...
    if uploaded_file is not None and uploaded_file.file_id != st.session_state.get('processed_file_id'):
        ...
        with st.status("🤖 Processing receipt...", expanded=True) as status:
            ...
            logs.insert(0, f"🎉 Completed! Added {added_count} transactions, skipped {skipped_count}")
            status.update(label=f"✅ Complete! Added {added_count}, skipped {skipped_count}", state="complete", expanded=True)
            log_placeholder.code("\n".join(logs))
